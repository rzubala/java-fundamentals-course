...
public class CrazyStreams {
    ...
    public int getNumOfLettersInFirstAndLastNames() {
    	//return (int)accounts.stream().collect(Collectors.summarizingInt(a -> a.getFirstName().length() + a.getLastName().length())).getSum();
    ...
    public List<Account> sortByFirstAndLastNames() {
        return accounts.stream().sorted((a, b) -> (a.getFirstName() + " " + a.getLastName()).compareTo(b.getFirstName() + " " + b.getLastName())).collect(Collectors.toList());
    }
    ...
    public BigDecimal getBalanceByEmail(String email) {
    	return accounts.stream().filter(a -> a.getEmail().equals(email)).map(a -> a.getBalance()).findFirst().orElseThrow(() -> new EntityNotFoundException("Cannot find Account by email="+email));
    }
    ...
    public Map<String, BigDecimal> collectBalancesByEmailForAccountsCreatedOn(int year) {
        return accounts.stream().filter(a -> a.getCreationDate().getYear() == year).collect(Collectors.toMap(Account::getEmail, Account::getBalance));
    }
    ...
    public Map<Month, BigDecimal> groupTotalBalanceByCreationMonth() {
    	return accounts.stream().collect(Collectors.groupingBy(a -> a.getCreationDate().getMonth(),
    			Collectors.mapping(Account::getBalance, Collectors.reducing(BigDecimal.ZERO, (a, b) -> a.add(b)))));
    }
    ...
    public Map<Character, Long> getCharacterFrequencyInFirstNames() {
    	return accounts.stream().map(Account::getFirstName).flatMapToInt(String::chars).mapToObj(c -> (char)c).collect(
